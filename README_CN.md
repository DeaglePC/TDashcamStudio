...
| 功能特性 | Tesla 原车播放器 | TeslaCam 播放器 (本项目) |
| :--- | :--- | :--- |
| **视角覆盖** | 仅 4 路 (前/后/左/右) | **完整 6 路同步** (含左右 B 柱视角) |
| **查看体验** | 仅限车内屏幕，操作略显局促 | **多端适配** (电脑/平板/手机)，大屏观看更清晰 |
| **视频处理** | 无法直接剪辑，需拷贝原始文件 | **可视化精确剪辑**，所见即所得 |
| **导出分享** | 导出原始分段视频，查看不便 | **一键合成网格视频**，带实时时间戳水印，方便社交分享/取证 |
| **行车数据** | 最新版有行车数据 | **实时仪表盘** (显示车速、踏板、AP状态、G值等) |
| **位置查找** | 简单显示位置 | **地图深度集成** (一键跳转高德/Google地图定位) |
| **隐私保护** | - | **100% 本地处理**，视频无需上传云端 |
...
### 📊 车辆元数据解析
*   **实时行车数据**: 自动解析视频流中嵌入的 SEI 元数据，实时显示车速、档位、方向盘角度、油门/刹车状态。
*   **安全与驾驶辅助**: 显示转向灯状态、自动驾驶（AP/FSD）激活状态。
*   **环境与动力学**: 显示 GPS 坐标、航向以及三轴加速度（X/Y/Z）。
